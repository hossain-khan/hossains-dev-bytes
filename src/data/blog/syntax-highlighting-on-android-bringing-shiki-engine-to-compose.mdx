...
One weekend I started wondering what syntax highlighting in Jetpack Compose would look like if I built it from scratch today - no `WebView`, no HTML template strings. I'd done the `WebView` + PrismJS/highlight.js approach [back in 2020](/posts/source-code-syntax-highlighting-on-android-taking-full-control) and it worked, but it always felt a bit awkward to embed a browser engine just to display colored text. Now that Compose is the default UI toolkit, I wanted something that fit in naturally.
...
The service is built with [Hono](https://hono.dev/) and runs on Cloudflare Workers. One interesting wrinkle: Shiki's default engine loads the Oniguruma WASM binary dynamically at runtime, which doesn't work in a Workers isolate. The Oniguruma WASM is also large enough (~1.5 MB) that it can push against the script size limit on Cloudflare's free tier. The fix was to switch to Shiki's JavaScript regex engine (`shiki/engine/javascript`), which uses native JS `RegExp` via [Oniguruma-To-ES](https://github.com/slevithan/oniguruma-to-es) transpilation instead of loading any WASM. No WASM needed, works in Workers, and cold starts are fast.
...
private fun parseHexColor(hex: String): Color {
    ...
    return when (clean.length) {
        // Parses only when it's likely valid HEX color 
        6, 8 -> Color(Color.parseColor("#$clean"))
...
While I was building the server-driven approach I also came across [kotlin-textmate](https://github.com/ivan-magda/kotlin-textmate) - a Kotlin/JVM port of [vscode-textmate](https://github.com/microsoft/vscode-textmate), the exact same tokenizer library VS Code uses. Since Shiki is built on the same TextMate grammar format and themes, the output quality is essentially the same. The difference is that everything runs on-device with zero network calls.
...
A few things to know before reaching for it: injection grammars aren't supported (so embedded languages like HTML inside JSX won't get sub-grammar highlighting), it isn't thread-safe (don't share a `Grammar` instance across coroutines without synchronization), and it retokenizes the full file on every call with no incremental state cache. For displaying static code snippets those limitations don't matter much - it works great. For a full code editor you'd want to add your own caching on top of `tokenizeLine()`.
