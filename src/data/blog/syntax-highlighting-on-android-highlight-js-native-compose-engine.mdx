...
The composable comes with a few useful parameters out of the box - a language badge in the header and a copy-to-clipboard button are on by default, while line numbers are off (pass `showLineNumbers = true` to enable them). The copy button does not show a built-in "Copied!" flash - you own that feedback. Pass an `onCopyClick` lambda to hook in a Snackbar, Toast, or your own animation. You can also supply a custom composable for the copy action via the `copyButton` slot.
...
viewModelScope.launch {
    engine.initialize().onFailure { /* handle WebView init failure */ } // warms up the WebView

    val result: Result<HighlightResult> =
        engine.highlight(code = "val x = 42", language = "kotlin", theme = HighlightTheme.tomorrow())
...
HighlightThemeProvider(
    ...
    val highlighted by rememberHighlightedCode(
        ...
        onHighlightComplete = { result ->
            // result.timings has per-stage Duration fields: jsBridge, htmlParse, treeWalk, etc.
...
- **Live highlighted editor (experimental)** - `SyntaxHighlightedTextEditor` and `rememberSyntaxHighlightedEditorValue` add a real-time editing experience with debounced highlighting while preserving cursor and selection.
...
Most code examples in this post still work as written. A few API names evolved over recent releases (for example custom copy slot naming and theme factory convenience APIs). For the latest API reference, see the [official docs](https://hossain-khan.github.io/android-compose-highlight/).
