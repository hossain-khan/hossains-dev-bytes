...
description: "How I added an AI post summarizer and Q&A assistant to my blog using Cloudflare Workers AI - and used AI Gateway to keep it mostly free."
...
After setting up my blog on [Astro](https://astro.build/) and [Cloudflare Workers](https://workers.cloudflare.com/) [recently](https://hossain.dev/posts/moving-away-from-medium-for-good/), I was wondering what else I can tinker with for my new blog that I have full control over 😆. 
I was aware that Cloudflare has free daily quota for AI models, so thought it would be fun to build a little AI assistant directly into the blog posts - something that could summarize the post and answer questions about it, all powered by Workers AI.
...
const stream = await env.AI.run(
  ...
  { gateway: { id: "my-blog" } },  // [!code highlight]
);
```
